        ...
        <div class="card-body" style="padding: 28px 30px;">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
                <div style="flex: 1; min-width: 200px;">
                    <form method="GET" action="{{ route('20pengumuman.index') }}" class="d-flex gap-2">
                ...
                <div class="d-flex flex-wrap align-items-center gap-2">
                    <div style="display: flex; align-items: center; gap: 6px;">
                        ...
                        <form method="GET" action="{{ route('20pengumuman.index') }}" id="perPageForm" class="d-inline">
                ...
                @forelse ($data as $index => $item)
                    <div class="card mb-4" style="border-radius: 16px; border: 1px solid #f0f2f5; box-shadow: 0 2px 8px rgba(0,0,0,0.02); overflow: hidden; transition: all 0.3s ease; background: #ffffff;">
                        <div class="card-body" style="padding: 20px 24px;">
                            ...
                            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
                                <div>
                                    <div class="d-flex align-items-center gap-3 flex-wrap">
                                        ...
                                        <h5 style="font-family: 'Poppins', sans-serif; font-weight: 700; color: #1a1a2e; margin: 0; font-size: 16px; word-break: break-word; max-width: 500px;">
                                            {{ Str::limit($item->sabha1 ?? 'Pengumuman', 100) }}
                                        </h5>
                                    </div>
                            ...
                            @php
                                $file = $item->sabha2;
                                $ext = $file ? pathinfo($file, PATHINFO_EXTENSION) : null;
                                $icon = 'mdi-file';

                                if ($ext) {
                                    if (in_array($ext, ['jpg','jpeg','png','gif','webp','bmp','svg','ico'])) $icon = 'mdi-image';
                                    elseif (in_array($ext, ['pdf'])) $icon = 'mdi-file-pdf';
                                    elseif (in_array($ext, ['doc','docx'])) $icon = 'mdi-file-word';
                                    elseif (in_array($ext, ['xls','xlsx'])) $icon = 'mdi-file-excel';
                                    elseif (in_array($ext, ['ppt','pptx'])) $icon = 'mdi-file-powerpoint';
                                    elseif (in_array($ext, ['zip','rar','7z'])) $icon = 'mdi-file-zip';
                                    elseif (in_array($ext, ['mp4','avi','mkv','mov'])) $icon = 'mdi-file-video';
                                    elseif (in_array($ext, ['mp3','wav','flac'])) $icon = 'mdi-file-music';
                                    else $icon = 'mdi-file';
                                }
                            @endphp

                            @if($item->sabha2)
                                <div style="background: #f8fafc; border-radius: 12px; padding: 12px 16px; border-left: 4px solid #c62828;">
                                    <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                                        <i class="mdi {{ $icon }}" style="font-size: 28px; color: #c62828;"></i>
                            ...
                            @else
                                <div style="text-align: center; padding: 16px; background: #f8fafc; border-radius: 12px; border: 1px dashed #e0e4ea;">
                            ...
                            <div class="modal-content" style="border-radius: 16px; border: none; box-shadow: 0 20px 60px rgba(0,0,0,0.15);">
                                ...
                                <div class="modal-body" style="padding: 20px 20px;">
                                    ...
                                    <p style="font-size: 13px; color: #7a8a9e; margin: 0; font-family: 'Poppins', sans-serif; word-break: break-word;">
                                        Judul: <strong>{{ Str::limit($item->sabha1 ?? 'Data', 50) }}</strong>
                                    </p>
                                    <p style="font-size: 12px; color: #c62828; margin-top: 8px; font-family: 'Poppins', sans-serif;">
                                        <i class="mdi mdi-alert"></i> File akan ikut terhapus!
                                    </p>
                                </div>
                ...
                @empty
                    <div style="text-align: center; padding: 60px 20px; background: #f8fafc; border-radius: 16px; border: 2px dashed #e0e4ea;">
                        ...
                        <h5 style="font-family: 'Poppins', sans-serif; font-weight: 500; color: #5a6a7a; margin: 0; font-size: 18px;">
                            @if(request('search')) Data Tidak Ditemukan! @else Belum Ada Pengumuman @endif
                        </h5>
                        <p style="font-family: 'Poppins', sans-serif; font-size: 14px; color: #b0b8c4; margin-top: 6px;">
                            ...
                            @else
                                Klik tombol <strong>"Tambah Pengumuman"</strong> untuk menambahkan pengumuman.
                            @endif
                        </p>
                    </div>
                @endforelse
            ...
            <form action="{{ route('pengumuman.create') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body" style="padding: 20px 24px;">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="sabha1" rows="4" placeholder="Masukkan judul atau isi pengumuman..." style="border-radius: 10px; font-family: 'Poppins', sans-serif; font-size: 14px; padding: 10px 14px;" required></textarea>
                    </div>

                    {{-- FILE (sabha2) --}}
                    <div class="mb-3">
                        <label class="form-label" style="font-family: 'Poppins', sans-serif; font-weight: 600; font-size: 14px;">
                            <i class="mdi mdi-file" style="color: #0d47a1;"></i> File (Gambar / PDF / Dokumen)
                        </label>
                        <input type="file" class="form-control" name="sabha2" style="border-radius: 10px; font-family: 'Poppins', sans-serif; font-size: 14px; padding: 8px 12px;">
                        <small style="font-family: 'Poppins', sans-serif; font-size: 12px; color: #888;">
                            <i class="mdi mdi-information-outline"></i> Semua jenis file (jpg, png, pdf, doc, xls, zip, mp4, dll) max 50MB
                        </small>
                    </div>
...
function exportToCSV() {
    ...
    var csv = [];
    var headers = ['No', 'Judul', 'File', 'Tanggal Dibuat'];
    ...
    dataCards.forEach(function(card, index) {
        ...
        var file = card.querySelector('.d-flex.align-items-center span');
        row.push(file ? file.textContent.trim() : '-');
    ...
    link.setAttribute('download', 'pengumuman_' + new Date().toISOString().slice(0,10) + '.csv');
