{{-- ============================================================
     DATA AGENDA EVENT FAMGATHERING (CARD ELEGAN - 5 FOTO)
     ============================================================ --}}
        ...
        <div class="card-body" style="padding: 28px 30px;">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
                <div style="flex: 1; min-width: 200px;">
                    <form method="GET" action="{{ route('13fam.index') }}" class="d-flex gap-2">
                        ...
                        @if(request('search'))
                            <a href="{{ route('13fam.index') }}" class="btn btn-outline-secondary" style="border-radius: 12px; font-family: 'Poppins', sans-serif; font-size: 13px; padding: 10px 20px; border-color: #e0e4ea; color: #5a6a7a;">
                ...
                <div class="d-flex flex-wrap align-items-center gap-2">
                    <div style="display: flex; align-items: center; gap: 6px;">
                        ...
                        <form method="GET" action="{{ route('13fam.index') }}" id="perPageForm" class="d-inline">
                        ...
                        <div class="modal-dialog modal-dialog-centered modal-sm">
                            <div class="modal-content" style="border-radius: 16px; border: none; box-shadow: 0 20px 60px rgba(0,0,0,0.15);">
                                ...
                                <div class="modal-footer" style="border-top: 1px solid #f0f2f5; padding: 12px 20px; gap: 8px;">
                                    ...
                                    <form action="{{ route('13fam.destroy', $item->id) }}" method="POST" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                ...
                @empty
                    <div style="text-align: center; padding: 60px 20px; background: #f8fafc; border-radius: 16px; border: 2px dashed #e0e4ea;">
                        ...
                        <h5 style="font-family: 'Poppins', sans-serif; font-weight: 500; color: #5a6a7a; margin: 0; font-size: 18px;">
                            @if(request('search')) Data Tidak Ditemukan! @else Belum Ada Agenda FAMGATHERING @endif
                        </h5>
                        <p style="font-family: 'Poppins', sans-serif; font-size: 14px; color: #b0b8c4; margin-top: 6px;">
                            ...
                            @else
                                Klik tombol <strong>"Tambah Agenda"</strong> untuk menambahkan agenda event FAMGATHERING.
                            @endif
                        </p>
                    </div>
                @endforelse
        ...
        <div class="modal-content" style="border-radius: 20px; border: none; box-shadow: 0 20px 60px rgba(0,0,0,0.15);">
            <div class="modal-header" style="border-bottom: 2px solid #c62828; padding: 20px 24px;">
                <h5 class="modal-title" style="font-family: 'Poppins', sans-serif; font-weight: 700; color: #1a1a2e; font-size: 20px;">
                    ...
                    Tambah Agenda FAMGATHERING
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{ route('fam.create') }}" method="POST" enctype="multipart/form-data">
                @csrf
        ...
        <div class="modal-content" style="border-radius: 20px; border: none; box-shadow: 0 20px 60px rgba(0,0,0,0.15);">
            <div class="modal-header" style="border-bottom: 2px solid #0d47a1; padding: 20px 24px;">
                <h5 class="modal-title" style="font-family: 'Poppins', sans-serif; font-weight: 700; color: #1a1a2e; font-size: 20px;">
                    ...
                    Edit Agenda FAMGATHERING
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{ route('fam.update', $item->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body" style="padding: 20px 24px;">
...
function exportToCSV() {
    ...
    link.setAttribute('download', 'famgathering_agenda_' + new Date().toISOString().slice(0,10) + '.csv');
